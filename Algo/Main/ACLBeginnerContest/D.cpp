...
int main() {
    ...
    SegmentTree<int> dp(MAX+1,[](int a,int b){return max(a,b);},0);     //dp[i]:=最後尾iとなるように条件を満たす部分列をとる。この時の長さの最大値
    rep(i,n){
        int upper=a[i]+k;
        int lower=a[i]-k;
        chmin(upper,MAX);
        chmax(lower,0);
        int val=dp.query(lower,upper+1);
        dp.chmax(a[i],val+1);
    }
    cout<<dp.query(0,MAX+2)<<endl;
}