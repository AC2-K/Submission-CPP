...
int main() {
    ...
    const int MAX=3001;
    vector<SegmentTree<mint>> dp(n+1,SegmentTree<mint>(MAX,[](mint a ,mint b){return a+b;},0));
    //@dp[i][j]=長さiで、back=jとなるcの個数
    dp[0].update(0,1);
    //セグメント木で高速化する.

    //dp[i][j]=dp[i-1]([0]+dp[1]+...+dp[j])
    for(int i=1;i<=n;i++){
        for(int j=a[i-1];j<=b[i-1];j++){
            mint sum=dp[i-1].query(0,j+1);
            dp[i].set(j,sum);
        }
        dp[i].build();
    }
    mint ans=dp[n].query(0,MAX+1);
    cout<<ans<<endl;
}